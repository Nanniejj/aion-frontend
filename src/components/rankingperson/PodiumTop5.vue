<template>
    <b-card no-body class="podium-card overflow-hidden">
        <!-- <top-accounts :accounts="items" :limit="10" :loading="loading" @filter-account="onFilterAccount" /> -->

        <!-- header -->
        <div class="podium-header d-flex justify-content-between align-items-center px-3 pt-3">
            <div class="title">
                <div class="h6 mb-0 text-white">Top20 Mentioned Persons</div>
    ...
    methods: {
        ...
        async fetchData() {
            try {
                ...
                this.items=Array.isArray(data) ? data : (data && (data.data || data.items)) || []
...
<style scoped>
.podium-card {
      background:linear-gradient(to top, #18bebea4, #faedb6);
