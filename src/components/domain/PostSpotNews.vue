<template>
  <div id="spotnews" class="container">
    <div class="mt-md-5 p-1">
      <div class="my-4 text-left">
        <h4 class="bold text-lg-left text-md-left">Hot Topics</h4>
       
        <div class="my-3 text-left h5">
    <span class="h5 bold">
      <i class="far fa-newspaper h5"></i> Topic :
    </span>

    <span v-if="topics.length <= 200">
      {{ topics }}
    </span>
    ...
    <template v-else>
      ...
      <button @click="showFull = !showFull" class="btn btn-link p-0" style="color: #8b8787;">
        {{ showFull ? 'ย่อข้อความ' : 'อ่านต่อ' }}
      </button>
    </template>
  </div>
      </div>
      <b-card class="card-platform">
        <!-- <div class="text-left text-tt mb-2 h6 " style="letter-spacing: 1.5px;">Total <span class="h5 bold">{{ count
          | numFormat }}</span>
          Posts</div> -->
        <b-row>
          <b-col cols="12" md="4" class="d-flex flex-column justify-content-center align-items-center">
            <div class="text-center text-tt mb-2 h6" style="letter-spacing: 1.5px;">
              Total
              <div class="h3 bold">{{ count | numFormat }}</div>
              Posts
            </div>
          </b-col>
            ...
            <b-card-body>
              <b-card-text class="box-contents">
                ...
                <Highlighter class="my-highlight md-font" :style="{
                  ...
                  fontSize: '17px',
                  padding: '10px',
                }" highlightClassName="highlight4" :searchWords="highlightText(heightword)" :autoEscape="true"
            ...
            " class="mb-4">
              ...
              <div v-else>
                ...
                <div v-else-if="datas.photos.length == 2" class="p-20">
                  <img class="images2" v-for="(image, i) in datas.photos" :src="image" @click="onClick(i, datas.photos)"
                    :key="`B-${i}`" onerror="this.style.display='none'" />
                </div>
                <div v-else-if="datas.photos.length == 3" class="p-20">
                  <img class="images3" v-for="(image, i) in datas.photos" :src="image" @click="onClick(i, datas.photos)"
                    :key="`C-${i}`" onerror="this.style.display='none'" />
                </div>
    ...
    <!-- <ul class="pagination" v-if="datacount != 0">
      <li class="page-item" v-for="pageNumber in totalPages" :key="pageNumber">
        <span v-if="
          ...
          pageNumber == 1
        ">
          <a class="page-link md-font" v-bind:key="pageNumber" href="#spotnews" @click="setPage(pageNumber)" :class="{
...
export default {
  ...
  methods: {
    ...
    apiSpotNewsPost() {
      ...
      this.axios(config)
        .then((response) => {
          var post = response.data.data[0].posts;
          this.topics=response.data.data[0].title||"";
          this.count = response.data.data[0].count || 0;
          this.datacount = response.data.data[0].filter_count || 0;
