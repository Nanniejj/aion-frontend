<template>
  <div>
    <div v-if="dataPost.length">
      ...
      <span v-for="(postDomain, k) in dataPost" :key="k">
        <b-row class="m-auto">
          <b-col lg="auto" class="d-contents num">
            <span class="print-text d-none">อันดับ </span>
                ...
                <b-row>
                  <b-col class="align-start col-lg-auto w-auto" sm="auto" lg="auto">
                    <span class="left">
                      ...
                      <img v-if="postDomain.source == 'pantip'" src="@/assets/Pantip.png" class="social-img" />
                  ...
                  <b-col class="align-start w-auto" sm="8" lg="auto">
                    <span id="txt-name">
                      ...
                      " v-bind:href="postDomain.url_post.replace('mbasic.', '')" class="fa fa-external-link"
                        target="_blank"></a>
                      <a v-else v-bind:href="postDomain.url_post" class="fa fa-external-link" target="_blank"></a>
                    </span>
...
">
    ...
    border-radius: 13px;">
              ...
              <template #footer>
                ...
                <!-- comment content -->
                <b-collapse :id="'btn' + page + k" class="mt-2"
...
export default {
  ...
  computed: {
    ...
    dataPost() {
      let data = [];
      if (this.typeStm == "pos") {
        data = this.getExportTopPostDomain;
      } else {
        data = this.getExportTopPostDomainNeg;
      }
      return data;
    },
  },
        ...
        () => {
          ...
          this.axios(config)
            .then(function (response) {
              ...
              if (v == 1) {
                _this.getExportTopPostDomain[k].sentiment = 1;
                _this.getExportTopPostDomain[k].user_sentiment[_this.objId] = 1;
              } else if (v == 0) {
                _this.getExportTopPostDomain[k].sentiment = 0;
                _this.getExportTopPostDomain[k].user_sentiment[_this.objId] = 0;
              } else {
                _this.getExportTopPostDomain[k].sentiment = -1;
                _this.getExportTopPostDomain[k].user_sentiment[
                  _this.objId
                ] = -1;
              }
            })
