<template>
  <div class="container mt-3">
    <div>
      ...
      <div id="tab-all" v-if="getLocationPost">
        <b-row class="mt-4">
          <b-col
            lg="8"
            class="m-auto my-1"
          ...
          <b-col class="text-lg-right my-1">
            <span id="post-comment" v-if="getLocationPost">
              <i class="far fa-paper-plane" />
              <b>
                <span v-if="getLocationPost && getLocationPost.count">
                  {{ getLocationPost.count | numFormat }} </span
      ...
      <div v-if="getLocationPost" class="container">
        <!-- Highlight -->
        <b-form-checkbox
          switch
          size="lg"
        ...
        <div
          v-if="
            getLocationPost &&
              getLocationPost.data &&
              getLocationPost.data.length == 0
          "
          class="md-font"
        >
          <b-card>
              ...
              <b-card-body>
                ...
                <b-card-text class="box-contents">
                  <Highlighter
                    ...
                    :searchWords="arrKeyword"
                    :autoEscape="true"
                    :textToHighlight="
                      datas.read
                        ? datas.full_text.slice(0, 450)
                        : datas.full_text
                    "
                  />
    ...
    border-radius: 13px;"
      ...
      <ul class="pagination" v-if="getLocationPost.length != 0">
        <li
          class="page-item"
      ...
      <input
        ...
        id="setpage"
        style="width: 150px"
        v-if="getLocationPost.length != 0"
      />

      <span v-if="getLocationPost.length != 0">
...
export default {
  ...
  computed: {
    ...
    paginate() {
      var data = this.getLocationPost.data;
      var count = this.getLocationPost.count;
  ...
  methods: {
    ...
    page() {
      var pageNumber;
      if (
        parseInt(this.gotopage) > Math.ceil(this.getLocationPost.count / 10) ||
    ...
    getTheSelected(k, v, uid) {
      ...
      this.$confirm("คุณต้องการเปลี่ยน Sentiment เป็น " + err + " ?").then(
        () => {
          ...
          this.axios(config)
            .then(function(response) {
              ...
              if (_this.selected == "1,0,-1") {
                if (v == 1) {
                  _this.getLocationPost.data[k].sentiment = 1;
                  _this.getLocationPost.data[k].user_sentiment[_this.objId] = 1;
                } else if (v == 0) {
                  _this.getLocationPost.data[k].sentiment = 0;
                  _this.getLocationPost.data[k].user_sentiment[_this.objId] = 0;
                } else {
                  _this.getLocationPost.data[k].sentiment = -1;
                  _this.getLocationPost.data[k].user_sentiment[
                    _this.objId
                  ] = -1;
                }
              } else {
                ...
                if (v == _this.selected) {
                  console.log("in 11");
                  _this.getLocationPost.data[k].sentiment = v;
                  _this.getLocationPost.data[k].user_sentiment[_this.objId] = v;
                } else {
                  console.log("in 12");
                  _this.getLocationPost.data.splice(k, 1);
                }
              }
            })
          ...
          // if (v !== this.getLocationPost[k].sentiment) {
          ...
          //   if (this.selected == "") {
          //     if (v == 1) {
          //       this.getLocationPost[k].sentiment = 1;
          //     } else if (v == 0) {
          //       this.getLocationPost[k].sentiment = 0;
          //     } else {
          //       this.getLocationPost[k].sentiment = -1;
          //     }
          //   } else {
          //     this.getLocationPost.splice(k, 1);
          //   }
          //   // this.getLocationPost.splice(k, 1);
          //   this.$fire({
          //     title: "แก้ไขสำเร็จ",
