        ...
        <b-row>
          <b-col>
            <div class="doc md-font">
              ...
              เอกสารที่พบ
              <strong> {{ getTiktok.post | numFormat }} </strong>
              (Comments/Posts)
            </div>
          </b-col>
        </b-row>
...
export default {
  ...
  computed: {
    ...
    startFormat() {
      var startDate = String(this.getSumTiktok.startdate)
        .replace("มกราคม", "ม.ค.")
    ...
    endFormat() {
      var endDate = String(this.getSumTiktok.enddate)
        .replace("มกราคม", "ม.ค. ")
        .replace("กุมภาพันธ์", "ก.พ. ")
        .replace("มีนาคม ", "มี.ค. ")
        .replace("เมษายน", "เม.ย. ")
  ...
  methods: {
    toPlatform() {
      ...
      this.$store.commit("setNamePlatform", "tiktok");
    },
  },
};
</script>
...
#tiktok {
  ...
  color: #ffffff;
  cursor: pointer;
  background: linear-gradient(145deg, #715a83 10%, #31203f 55%);
