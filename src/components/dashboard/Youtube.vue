<template>
  <b-col sm="12" md="6" lg="4">
    ...
    <b-modal id="modal-6" size="lg" centered>
      <b-container>
        <b-row>
...
export default {
  ...
  computed: {
    ...
    startFormat() {
      var startDate = String(this.getSumYoutube.startdate)
        .replace("มกราคม", "ม.ค.")
    ...
    endFormat() {
      var endDate = String(this.getSumYoutube.enddate)
        .replace("มกราคม", "ม.ค. ")
        .replace("กุมภาพันธ์", "ก.พ. ")
        .replace("มีนาคม ", "มี.ค. ")
        .replace("เมษายน", "เม.ย. ")
    ...
    startWeek() {
      var sweek = new Date(this.getTimelineYoutube[1][0]).toLocaleDateString(
        "en-AU"
      );
      return sweek;
    },
    endWeek() {
      var eweek = new Date(this.getTimelineYoutube[8][0]).toLocaleDateString(
        "en-AU"
      );
      return eweek;
    },
    startMonth() {
      var smonth = new Date(this.getTimelineYoutube[1][0]).toLocaleDateString(
        "en-AU"
      );
      return smonth;
    },
    endMonth() {
      var emonth = new Date(
        this.getTimelineYoutube[this.getTimelineFacebook.length - 1][0]
  ...
  methods: {
    toPlatform() {
      ...
      this.$store.commit("setNamePlatform", "youtube");
    },
  },
};
</script>
