<template>
  <b-col sm="12" md="6" lg="4">
    ...
    <vue-element-loading
      ...
      color="#b6ac9a"
    />
    <div class="box" id="threads" @click="toPlatform">
      <div class="right"></div>
      <!-- <i class="fab fa-instagram fa-2x"></i> -->
      <img src="@/assets/Threads.png" class="socialogo" />
      <div class="md-font">
        โพสต์ที่เกี่ยวข้อง
        <span class="total-all"> {{ getThreads.post | numFormat }} </span>
      </div>
      <b-container>
        <b-row class="row comment-post">
          ...
          <b-col class="border-left">
            ...
            <div class="total-sub">
              ...
              {{ getThreads.comment + getThreads.post | numFormat }}
            </div>
          </b-col>
        </b-row>
        ...
        <b-row class="md-font">
          ...
          <b-col cols="7" class="sum-right" >
            <div>
              ...
              {{ getSumThreads.post | numFormat }}
            </div>
            <div>
              <span class="prt">Comments : </span>
              {{ getSumThreads.comment | numFormat }}
            </div>
            <div>
              <span class="prt">Users : </span>
              {{ getSumThreads.users | numFormat }}
            </div>
            <div>
    ...
    <b-modal id="modal-5" size="lg" centered>
      <b-container>
        ...
        <b-row>
          <b-col>
            <div class="doc md-font">
              ...
              เอกสารที่พบ
              <strong> {{ getThreads.post | numFormat }} </strong>
              (Comments/Posts)
            </div>
          </b-col>
        </b-row>
        ...
        <SentimentChart
          v-if="getThreads.total_sentiments"
          :source="'instagram'"
          :chartData="getThreads.total_sentiments"
          :pageType="'DashboardPage'"
        />
        <!-- <SentimentChart :chartData="getThreads.sentiment" /> -->
      </b-container>
    </b-modal>
  </b-col>
</template>
...
export default {
  ...
  computed: {
    ...
    startFormat() {
      var startDate = String(this.getSumThreads.startdate)
        .replace("มกราคม", "ม.ค.")
    ...
    endFormat() {
      var endDate = String(this.getSumThreads.enddate)
        .replace("มกราคม", "ม.ค. ")
        .replace("กุมภาพันธ์", "ก.พ. ")
        .replace("มีนาคม ", "มี.ค. ")
        .replace("เมษายน", "เม.ย. ")
  ...
  methods: {
    toPlatform() {
      ...
      this.$store.commit("setNamePlatform", "threads");
    },
  },
};
</script>
...
<style scoped>
#threads {
  background-color: #3f2e4c;
  color: #ffffff;
  cursor: pointer;
  /* background: linear-gradient(145deg, #ff9d4d 10%, #a423bd 55%); */
  background: linear-gradient(145deg, #e75aa1 10%, #910c64 55%);
