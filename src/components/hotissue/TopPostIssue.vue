<template>
  <div>
    <div v-if="dataIssue.length">
      ...
      <span v-for="(postDomain, k) in dataIssue" :key="k">
        <b-row class="m-auto">
          ...
          <b-card class="mb-4" style="border: none" id="box-content" footer-tag="footer" header-tag="header">
            <vue-element-loading :active="getLoadPostIssue" size="80" background-color="rgba(255, 255, 255, 0.5)"
              color="#b6ac9a" />
            <!-- {{ k }} -->
            <template #header v-if="postDomain">
              <b-row>
                ...
                <b-col class="align-start w-auto" sm="8" lg="auto">
                  <span id="txt-name">
                    ...
                    " v-bind:href="postDomain.url_post.replace('mbasic.', '')" class="fa fa-external-link"
                      target="_blank"></a>
                    <a v-else v-bind:href="postDomain.url_post" class="fa fa-external-link" target="_blank"></a></span>
                ...
                <b-col class="text-md-right" sm="12" lg="" md="">
                  ...
                  <i v-if="postDomain.snapshot" class="fas fa-camera mr-2" @click="onClick(0, [postDomain.snapshot])" />
            ...
            <b-row>
              <b-col md="12"> </b-col>
              <b-col lg="12">
                <!-- {{postDomain.hotissue_docs.keywords}} -->
                <div id="txt-cmt" class="font-weight-normal" v-if="postDomain.full_text">
                  <div class="box-contents">
                    ...
                    <Highlighter class="my-highlight" :style="{
                      ...
                      fontSize: '17px',
                      padding: '10px',
                    }" highlightClassName="highlight2" :searchWords="heightword" :autoEscape="true" :textToHighlight="postDomain.read
                    ...
                    <div v-if="postDomain.full_text.length > 450" @click="postDomain.read = !postDomain.read"
                      id="readmore">
                      <span v-if="postDomain.read == true">... อ่านต่อ</span><span v-else>ย่อบทความ</span>
                    </div>
                  </div>
                </div>
              </b-col>
              <b-col>
                <div v-if="postDomain.source == 'tiktok' && postDomain.uid">
                  <a v-bind:href="postDomain.url_post" target="_blank">
                    <lite-tiktok :videoid="postDomain.uid" style=" pointer-events: none; "></lite-tiktok></a>
...
">
            ...
            <template #footer>
              <div class="text-left md-font">
                ...
                ">
                  ...
                  <span v-if="
                    postDomain.shares_count !== '0' && postDomain.shares_count
                ...
                <span v-if="postDomain.reaction">
                  <span v-if="postDomain.reaction != ''">
                    ...
                    <span v-if="postDomain.reaction.share">
                      <span v-if="postDomain.reaction.share !== '0'" id="box-reaction" v-b-tooltip.hover title="Share">
                    ...
                    <span v-if="postDomain.reaction.shares">
                      <span v-if="postDomain.reaction.shares !== '0'" id="box-reaction" v-b-tooltip.hover title="Share">
                    ...
                    <span v-if="postDomain.reaction.Angry">
                      <span v-if="postDomain.reaction.Angry !== '0'" id="box-reaction" v-b-tooltip.hover title="Angry">
              ...
              <!-- comment content -->
              <b-collapse :id="'btn' + page + k" class="mt-2" v-if="postDomain.comments && postDomain.comments.length">
                      ...
                      <b-row>
                        ...
                        <b-col lg="11">
                          <div>
                            ...
                            <span v-if="postDomain.source == 'youtube' && cmt.time" class="font-weight-light"
    ...
    <div v-else>
      <b-card class="mb-4" footer-tag="footer" header-tag="header">
        <vue-element-loading :active="getLoadPostIssue" size="80" background-color="rgba(255, 255, 255, 0.5)"
          color="#b6ac9a" />
...
export default {
  ...
  methods: {
    ...
    async getData(name, date, social) {
      this.$store.commit("setLoadPostIssue", true);
      ...
      var config = {
        ...
        url:
          API_V2_URL +
          "/api/v2/userposts/getPostHotissue?source=" +
          social +
          "&offset=0&name=" +
          name +
          sdate +
      ...
      await axios(config)
        .then((response) => {
          if (response && response.data) {
            ...
            //     name: "Posts",
            //     data: [
            //       response.data[0].platform_counts.facebook,
            //       response.data[0].platform_counts.twitter,
            //       response.data[0].platform_counts.pantip,
            //       response.data[0].platform_counts.news,
            //       response.data[0].platform_counts.youtube,
            //       response.data[0].platform_counts.instagram,
            //       response.data[0].platform_counts.blockdit,
            //       response.data[0].platform_counts.tiktok,
            //       response.data[0].platform_counts.threads,
            //     ],
            //   },
            //   {
            //     name: "Comments",
            //     data: [
            //       response.data[0].platform_counts.facebook_comments,
            //       response.data[0].platform_counts.twitter_comments,
            //       response.data[0].platform_counts.pantip_comments,
            //       response.data[0].platform_counts.news_comments,
            //       response.data[0].platform_counts.youtube_comments,
            //       response.data[0].platform_counts.instagram_comments,
            //       response.data[0].platform_counts.blockdit_comments,
            //       response.data[0].platform_counts.tiktok_comments,
            //       response.data[0].platform_counts.threads_comments,
            //     ],
            //   },
            // ]
            console.log("series", series);
            this.$store.commit("setChartDataIssue", series);
          }
          let heightarr = [];
          let andarr = [];
          let splitarr = [];
  ...
  async created() {
    ...
    // this.$store.commit('setSocialIssue',"facebook,twitter,pantip")
