...
export default {
  ...
  computed: {
    ...mapGetters(["getSelectIssue", "getDateIssue", "getEdateDm", "getSocialIssue","getStatusSearch"]),
  },
  ...
  methods: {
    ...
    apiFilterChart(start,name,date,social) {
      ...
      if(social){
        socials = "&source=" + social;
      }

// "https://api2.cognizata.com/api/v2/userposts/getChartHotissue?name="
      var config = {
        method: "get",
        url:
          "http://localhost:3000/api/v2/userposts/getChartHotissue2?name=" +
         name +
          sdate +
          edate +
          label+socials+"&sentiment=1,0,-1",
        headers: {
  ...
  mounted() {
    ...
    this.apiFilterChart("",name, arrdate, social);
