<template>
  <div>
    <!-- {{ loadWord }} -->
    <vue-element-loading :active="loadWord" size="80" background-color="rgba(255, 255, 255, 0.4)" color="#b6ac9a" />
    <b-row id="input-btn">
    ...
    <div>
      ...
      <b-table v-if="dataKeyword && dataKeyword.subdomains && dataKeyword.subdomains.length"
        :items="dataKeyword.subdomains[0].objects" :fields="fields" :current-page="currentPage" :per-page="perPage"
        :filter="filter" :filter-included-fields="['name', 'keywords']" :sort-by.sync="sortBy"
        :sort-desc.sync="sortDesc" :sort-direction="sortDirection" :empty-filtered-text="empData" stacked="md"
        show-empty small @filtered="onFiltered" head-variant="light">
        ...
        <template #cell(domain)="row">
          ...
          {{ dataKeyword.domain_name }}
        </template>
        <template #cell(subdomain)="row">
          {{ dataKeyword.subdomains[0].subdomain_name }}
        </template>
    ...
    <b-row>
      <b-col sm="7" md="6" class="my-1" id="page" v-if="!filter">
        <b-pagination @input="setPage" v-model="currentPage" :total-rows="dataKeyword.subdomains[0].objects.length || 0"
          :per-page="perPage" align="fill" size="sm" class="my-0"></b-pagination>
      </b-col>
      <b-col sm="7" md="6" class="my-1" id="page" v-else>
        <b-pagination @input="setPage" v-model="currentPage" :total-rows="totalRows" :per-page="perPage" align="fill"
          size="sm" class="my-0"></b-pagination>
      </b-col>
    </b-row>

    <!-- Info modal -->
    <vue-modaltor :visible="open" @hide="hideModal" :resize-width="{ 3500: '50%', 1500: '70%', 992: '70%', 768: '90%' }"
      ...
      <b-container fluid>
        ...
        <div>
          ...
          <b-col sm="12">
            <b-form-input id="input-small3" placeholder="" v-model="textWord" maxlength="50"></b-form-input>
          ...
          <b-tabs content-class="mt-3" lazy active-nav-item-class="font-weight-bold "
            active-tab-class="font-weight-bold " v-model="tabIndex">
            <b-tab>
              <template #title calss="title-tab">
                <span id="title-tab">Keyword</span></template>
              <b-form-tags v-model="textKeyword" tag-variant="light" tag-pills size="md"
                placeholder="Enter เพื่อพิมพ์คำใหม่" remove-on-delete></b-form-tags>
            </b-tab>
            <b-tab>
              <template #title calss="title-tab">
                <span id="title-tab">Include Keyword</span>
              </template>
              <b-form-tags v-model="textInclude" tag-variant="light" :tag-validator="validator" @tag-state="onTagState"
                tag-pills size="md" placeholder="Enter เพื่อพิมพ์คำใหม่" remove-on-delete></b-form-tags>
            ...
            <b-tab>
              ...
              <b-form-tags v-model="textExclude" tag-variant="light" tag-pills size="md"
                placeholder="Enter เพื่อพิมพ์คำใหม่" remove-on-delete></b-form-tags>
            </b-tab>
          </b-tabs>
          <br />
...
import CreateObject from "@/components/setting_page/CreateObject.vue";
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "object_name", label: "Object", sortable: true },
  ...
  methods: {
    async apiList() {
      ...
      try {
        const config = {
          ...
          params: { domain_id: this.$route.query.id, subdomain_id: this.$route.query.subdomain_id },
          headers: {
        ...
        this.dataKeyword = response.data
    ...
    info(item, index, button) {
      ...
      this.idWord = item.object_id;
      this.IdDm = this.$route.query.id;
      this.subDomainId = this.$route.query.subdomain_id;
      this.infoModal.content = this.$route.params.domain;
      this.textWord = item.object_name;
  ...
  async mounted() {
    this.apiList()
    await this.$emitter.on("callApiListSubdomain", () => {
      this.apiList()
    });
    // if (this.getSubDomainName.domain.name) {
    //   this.$store.dispatch("fetchListWord", {
    //     domain: this.getSubDomainName.domain.name,
    //     subdomain: this.getSubDomainName.name,
    //     offset: this.offset,
    //   });
    // } else {
