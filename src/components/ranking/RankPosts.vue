<template>
  ...
  <div id="wordcloud">
      ...
      <b-row v-if="getQuerySearch" class="mb-1 box-stitle">
        <b-col md="8" lg="8" class="m-auto my-1">
          <h5 class="bold text-lg-left text-md-left">
            {{ getObjectName }}
            <!-- <span v-if="getSubDomain"> / {{getSubDomain}}</span>  -->
            <span v-if="getObName"> / {{ getObName }}</span>
          </h5>
        </b-col>
        <b-col md="4" lg="4" class="text-lg-right my-1">
          <span id="post-comment">
            ...
            <b>
              <span v-if="getSentimentDetailRanking.count">
                {{ getSentimentDetailRanking.count | numFormat }} </span
        ...
        <b-col sm="12" md="12" lg="6" id="tab-post" v-if="!checkpost">
          <b-row>
            <b-col cols="12">
              ...
              <span id="post-comment">
                <i class="far fa-paper-plane" />
                <b> {{ getSentimentDetailRanking.count | numFormat }} </b> posts
                |
                <i class="far fa-comments" />
                <b>
                  {{ getSentimentDetailRanking.total_comment | numFormat }}
                </b>
                comments</span
              >
            </b-col>
    ...
    <div v-if="getSentimentDetailRanking">
      <!-- Highlight -->
      <b-form-checkbox
        switch
        size="lg"
      ...
      >
        <span :style="myStyle" v-if="checked" class="box-hl pl-2 pr-2"
          >Highlight</span
        >
        <span v-else class="box-hl pl-2 pr-2">Highlight</span></b-form-checkbox
      >
      <b-card
        no-body
    ...
    <ul class="pagination" v-if="getDetailPostRanking.length != 0">
    ...
    <input
      ...
      id="setpage"
      style="width: 150px"
      v-if="getDetailPostRanking.length != 0"
    />

    <span v-if="getDetailPostRanking.length != 0">
...
export default {
  ...
  computed: {
    ...
    // },
    paginate() {
      var data = this.getDetailPostRanking;
      var count = this.getSentimentDetailRanking.count;
    ...
    page() {
      ...
      if (
        parseInt(this.gotopage) >
          Math.ceil(this.getSentimentDetailRanking.count / 10) ||
    ...
    getTheSelected(k, v, uid) {
      ...
      this.$confirm("คุณต้องการเปลี่ยน Sentiment เป็น " + err + " ?").then(
        () => {
            ...
            .then(function(response) {
              ...
              if (_this.selected == "") {
                if (v == 1) {
                  ...
                  _this.getDetailPostRanking[k].user_sentiment[_this.objId] = 1;
                } else if (v == 0) {
                  _this.getDetailPostRanking[k].sentiment = 0;
                  _this.getDetailPostRanking[k].user_sentiment[_this.objId] = 0;
                } else {
                  _this.getDetailPostRanking[k].sentiment = -1;
                  _this.getDetailPostRanking[k].user_sentiment[
                    _this.objId
                  ] = -1;
                }
              } else {
          ...
          // if (v !== this.getDetailPostRanking[k].sentiment) {
          ...
          //   if (this.selected == "") {
          //     if (v == 1) {
          //       this.getDetailPostRanking[k].sentiment = 1;
          //     } else if (v == 0) {
          //       this.getDetailPostRanking[k].sentiment = 0;
          //     } else {
          //       this.getDetailPostRanking[k].sentiment = -1;
          //     }
          //   } else {
          //     this.getDetailPostRanking.splice(k, 1);
          //   }
          //   // this.getDetailPostRanking.splice(k, 1);
          //   this.$fire({
          //     title: "แก้ไขสำเร็จ",
