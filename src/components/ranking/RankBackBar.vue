<template>
  <div>
    ...
    <div id="flowPhone">
      <div id="flow-tt">
        <div @click="backranking()" style="cursor: pointer"><a>Ranking</a></div>
        <div @click="backranking()" style="cursor: pointer; margin-left: 5px">
          <i class="fas fa-chevron-right"></i>
          <span
            >{{ getObjectName }}
            <span v-if="getSubDomain"> / {{ getSubDomain }}</span></span
          >
        </div>
      </div>
      <div id="key">
        <a style="margin-left: 5px; color: #343a40"
          ><i class="fas fa-chevron-right"></i> <b>{{ getObName }}</b> |</a
        >
    ...
    <div id="flowBoxes">
      ...
      >
        <a style="margin-left: 18px; color: #4c412b">
          <span v-if="getObjectName == ''"
            >ALL
            <span v-if="getSubDomain"> / {{ getSubDomain }}</span><span class="prt"> /</span></span
          >
          <span v-else
            >{{ getObjectName }}
            <span v-if="getSubDomain"> / {{ getSubDomain }}</span><span class="prt"> /</span></span
      ...
      <div class="leftt rightt active">
        <span style="margin-left: 18px">{{ getObName }}</span><span class="prt"> /</span>
      </div>
