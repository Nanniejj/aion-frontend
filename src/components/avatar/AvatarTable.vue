  ...
  <div class="table" v-if="getSummaryAvatar">
   <b-row class="mb-3"> 
     <b-col lg="6" class="my-1">
      ...
      <b-col class="my-1">
       <button class="btn btn-add pl-3 pr-3 font-weight-bold" @click="goAvatarTeam"> <i class="fas fa-user-cog "/>
   ...
   <b-row class="mb-4 mt-4" cols="2" cols-md="4" cols-lg="4" v-if="selected=='twActive'">
       <b-col>
         <i class="fa fa-twitter"></i>
         <div><b>{{getSummaryAvatar.twitter.tweet |numFormat}}</b> Tweets </div>
       </b-col>
        <b-col>
          <i class="fa fa-retweet"></i>
         <div><b>{{getSummaryAvatar.twitter.retweet |numFormat}}</b> Retweets</div>
       </b-col>
        <b-col>
          <i class="fas fa-reply"></i>
         <div><b>{{getSummaryAvatar.twitter.reply |numFormat}}</b> Reply</div>
       </b-col>
        <b-col>
          <i class="fas fa-heart"></i>
         <div><b>{{getSummaryAvatar.twitter.favourites |numFormat}}</b> Likes</div>
       </b-col>
        <!-- <b-col>
    ...
    <b-row class="mb-4 mt-4" v-else cols="3" cols-md="3" cols-lg="3">
       <b-col>
         ...
         <div><b>{{getSummaryAvatar.facebook.post |numFormat}}</b> Posts </div>
       </b-col>
        <b-col>
          <i class="fa fa-comments"></i>
         <div><b>{{getSummaryAvatar.facebook.comment |numFormat}}</b> Comments</div>
       </b-col>
        <b-col>
          <i class="fas fa-share"></i>
         <div><b>{{getSummaryAvatar.facebook.share |numFormat}}</b> Share</div>
       </b-col>
   </b-row>
  </div>
     ...
     >
      ...
      <template #cell(total_account)="row">
       ...
       <template #cell(active)="row">
        ...
        <div>{{ row.item.twitter.total_account_active |numFormat }}</div>
          <b-button size="sm" @click="row.toggleDetails" id="btn-show" class="ml-3" >
        ...
        <div v-if="rows.item.twitter.export_data.length" class='user-card' >
          ...
          :borderless="true"
           ...
           <template #cell(view)="row">
              <span class="fas fa-list-ul float-right" v-b-tooltip.hover title="ดูข้อมูล" size="sm" @click="open = true;info(row.item,rows.item.name);" ></span>
           </template>
      ...
      <template #cell(likes)="row">
        ...
        <template #cell(top3_hashtag)="row" >
          ...
          <span v-if="row.item.twitter.top3_hashtag.name[0]"><b>1 </b>{{row.item.twitter.top3_hashtag.name[0]}}</span>
          <br><span v-if="row.item.twitter.top3_hashtag.name[1]"><b>2 </b>{{row.item.twitter.top3_hashtag.name[1]}}</span>
          <br><span v-if="row.item.twitter.top3_hashtag.name[2]"><b>3 </b>{{row.item.twitter.top3_hashtag.name[2]}} </span></div>
...
<!-- -----------------------------------facebook table---------------------------------------------------- -->
    ...
    <b-row  class="mt-5 text-center mb-4">
      <b-col sm="7" md="6" class="my-1 m-auto " id='page' v-if="filterTw||filterFb">
            <b-pagination
              v-model="currentPage"
 ...
 <!-- ----------------------------------------------Info modal--------------------------------------------- -->
    ...
    :animation-panel="'modal-slide-top'" class="hei-popup" >
     <b-container fluid>
       <div v-if="selected=='twActive'" > 
       <div class="hh">
         <a :href="'https://twitter.com/'+dataInfo.account_name" target="_blank" id="acclink">
         <span class="bold h5" >
           <span v-if="dataInfo.profile_image"><img :src="dataInfo.profile_image" id="profile-pic"></span>
           <span v-else ><b-avatar class="mr-3"></b-avatar></span> {{dataInfo.account_name}} 
         </span>
         <i class="fa fa-external-link mr-2"/>
         </a>
         <span class="h6"> <b-badge id="box-team" >{{teamName}}</b-badge></span>
      </div> <hr>    
       <b-row class="text-center" cols="2" cols-sm="2" cols-md="2" cols-lg="4" >
       <b-col  >
         <div ><i class="fa fa-users" style="color: rgb(100, 103, 105);"></i></div>
         <div><b>{{dataInfo.follower |numFormat}}</b> Followers</div>
       </b-col>
        <b-col > 
          <div><i class="fa fa-twitter" style="color: rgb(29, 161, 242);"></i></div>
         <div><b>{{dataInfo.tweet |numFormat}}</b> Tweets </div>
       </b-col>
        <b-col >
          <div><i class="fa fa-retweet" style="color:rgb(23, 191, 99);"></i></div>
         <div><b>{{dataInfo.retweet |numFormat}}</b> Retweets</div>
       </b-col>
        <b-col >  
          <div><i class="fa fa-heart" style="color: rgb(224, 36, 94);"></i></div>
         <div><b>{{dataInfo.likes_count |numFormat}}</b> Likes</div>
       </b-col>
     </b-row>
     <div class="pl-3" >
      <div class="bold mt-3 ">Hashtags <span class="small">( total : <b>{{dataHash.length |numFormat}}</b> hashtags )</span></div> 
      ...
      <div class="mt-3 bold">Tweets  <span class="small">( total : <b>{{uidLink.length |numFormat}}</b> tweets )</span></div>
      <span v-if="uidLink.length">
      <div v-for="(post,k) in uidLink" :key="k" >
         <a
         id="box-links"
         class="mt-2"
         :href="'https://twitter.com/'+dataInfo.account_name+'/status/'+post"
        target="_blank"
         >
         <span>{{k+1}}. </span>
         https://twitter.com/{{dataInfo.account_name}}/status/{{post}} <i class="fa fa-external-link"/></a>
   ...
   watch: {
    ...
    return {
      ...
      {key:'account_name',label:'Account'},
       'follower', 'tweet','retweet',{key:'likes_count',label:"Likes"},{key:'view',label:""}],
      selected: 'twActive',
        options: [
...
div.modal-vue-panel.modal-slide-top.modal-vue-show > div.modal-vue--content.space-content > div > div > div > div.pl-3{
