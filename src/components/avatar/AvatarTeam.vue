      ...
      <b-row class="mb-3">
        ...
        <b-col class="my-1 text-right">
          <div class="d-inline mr-2">
            <button @click="showCheck" class="btn btn-add">
              เลือกหลายรายการ
            </button>
          </div>
          <CreateMonitor
            class="text-right d-inline"
            :tabsMonitor="'tabAvatar'"
          />
        </b-col>
      </b-row>
    </b-container>
      ...
      <b-row>
        ...
        <b-col>
          <div class="text-right mt-4 mb-2">
            <span class="mr-3 tagfilter pr-3 pl-3" v-if="filterUser"
              >ค้นหา <b>{{ filterUser }}</b> พบ <b>{{ totalRows }}</b> บัญชี
            </span>
            <span
              >มีทั้งหมด <b>{{ getAvatarTeam.length }}</b> ทีม |
              <b>{{ getAvatarUser.length }}</b> บัญชี</span
            >
          </div>
        </b-col>
      </b-row>
        ...
        <template #cell(link_original)="row">
          <div v-if="row.item.source == 'twitter'" class="w-auto m-auto">
            <a :href="row.value" target="_blank" id="link_tw">
              <img src="@/assets/Twitter.png" class="twicon" />
              {{ row.value }}</a
            >
          </div>
...
export default {
  ...
  watch: {
    ...
    getAvatarUser: function() {
      // watch it
    },
    getAvatarTeam: function(newVal, oldVal) {
      // watch it
      console.log("Prop changed: ", newVal, " | was: ", oldVal);
    },
  },
  ...
  computed: {
    ...
    sortOptions() {
      ...
      return this.FieldsTeam.filter((f) => f.sortable).map((f) => {
