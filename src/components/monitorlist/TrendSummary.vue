<template>
    <b-card header-class="text-left">
        <template #header>
            ...
            <!-- <button variant="info" size="sm" class="analysis-button" :class="{ 'is-glowing': analyzing }" type="button"
                @click="summarizeGraph()" :disabled="analyzing">
                <img width="22" height="22" src="https://img.icons8.com/ios-filled/50/sparkling--v1.png" alt="sparkling"
                    class="spark-icon" style="filter: brightness(0) invert(1);" />
                <span class="md-font">
        ...
        <div v-if="summary && summary.text" class="text-left card-body-scroll">
            ...
            <div v-if="fullSummary">
                ...
                <div class="text-left pb-3">
                    <ul class="mb-0 pl-4 ">
                        <li class="bold">วิเคราะห์ช่วงเวลาที่มี Engagement สูงสุด ({{
                            fmtNum(fullSummary.peak_engagement_summary.engagement_value || 0) }})</li>
                        <span>{{ fullSummary.peak_engagement_summary.interpretation }}<br></span>
                        <span class="bold">ประเด็น : </span>
                        <b-badge variant="warning" v-for="item in fullSummary.peak_engagement_summary.key_drivers"
                            :key="item" class="mr-2 mb-2 badge-wrap">
                            {{ item }}
                        </b-badge>
                        <li class="bold">วิเคราะห์ช่วงเวลาที่มีจำนวน Post สูงสุด ({{
                            fmtNum(fullSummary.peak_post_count_summary.post_count_value || 0) }} โพสต์)</li>
                        <span>{{ fullSummary.peak_post_count_summary.interpretation }}<br></span>
                        <span class="bold">ประเด็น : </span>
                        <b-badge variant="warning" v-for="item in fullSummary.peak_post_count_summary.key_drivers"
                            :key="'index - ' + item" class="mr-2 mb-2 badge-wrap">{{ item }}</b-badge>
                    </ul>
                </div>
    ...
    watch: {
        filters: {
            handler(newVal, oldVal) {
                // ถ้า data เดิม-ใหม่เหมือนกัน ไม่ต้องทำงาน
                if (JSON.stringify(newVal) === JSON.stringify(oldVal)) return;

                // ถ้าไม่มี start หรือ end ไม่ต้อง summarize
...
.latest-summary {
    ...
    /* อนิเมชันเปลี่ยนสี (ไม่จำเป็นแต่สวย) */
}
...
.latest-summary:hover {
    ...
    /* ความหนาของขีด (optional) */
    text-underline-offset: 3px;
    /* ระยะห่างของขีดใต้จากตัวอักษร (optional) */
    color: #17a2b891;
    /* สีเมื่อ hover */
}
