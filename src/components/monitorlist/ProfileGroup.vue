        ...
        <b-row id="timeline-container" class="m-0">
            <b-col id="chart" cols="12" class="px-0">
                ...
                <div class="position-relative text-right filter-btn">
                    <b-button size="sm" :variant="showFilters ? 'info' : 'outline-info'"
                ...
                <b-card v-if="showFilters" class="mb-3 shadow-sm" style="border-radius: 20px;">
                    ...
                    <b-tooltip target="info-date-note" placement="bottom">
                        ...
                        <div class="text-left"> <b-icon icon="info-circle" class="" variant="info"></b-icon>
                            <small>คำค้นหา (AND
                                ใช้ช่องว่างหรือ +, OR
                                ใช้ ,)
                        ...
                        <b-row>
                            ...
                            <b-col cols="12" md="4">
                                ...
                                <b-form-group class="pr-md-3 checkbox-v">
                                    <b-form-checkbox-group v-model="formFilters.sentiment"
                            ...
                            <b-col cols="12" md="8" class="pr-md-0">
                                <b-form-group label="" label-for="accounts" class="pr-md-3">
                                    <b-form-tags :disabled="loadingTimeline" id="accounts"
                                        v-model="formFilters.accountsInput" tag-variant="light" tag-pills size="md"
                                        separator=",;" placeholder="ค้นหาบัญชี (ใส่ uid หรือ url หลายบัญชีคั่นด้วย ,)"
                                        no-add-on-enter add-on-change remove-on-delete />
                                </b-form-group>
                            </b-col>

                            <b-col cols="12" md="4">
                                <b-form-select :disabled="loadingTimeline" v-model="formFilters.source" class="mb-2"
                                    :options="sourceOptions" />
                            </b-col>

                            <b-col cols="12" md="4">

                                <b-form-select v-model="formFilters.sort_by" class="mb-2" :disabled="loadingTimeline"
                                    :options="[
                                ...
                                <b-row align-h="end" class=" mt-2 mt-md-0 mx-0" justify="center">
                                    <b-col cols="auto" md="auto" class="px-0">
                                        <div class="align-self-end">
                                            <b-button type="submit" size="sm" variant="info" class="mr-2 px-4"
                        ...
                        <span v-if="pretty.sentiments.length !== 3" class="">
                            <b-badge pill variant="light" class="mr-2 py-2 my-1">
                                <span v-for="(s, i) in pretty.sentiments" :key="s.value">
                                    <b-icon :icon="s.icon" class="mr-1"></b-icon>{{ s.text }} <span v-if="i > 1"> /
                                    </span>
                                </span>
            ...
            <b-col cols="12" class="text-center mt-10 py-4 pb-0" v-if="loadingChart">
                <vue-element-loading :active="loadingChart" size="30" background-color="rgba(255, 255, 255, 0.5)"
            ...
            <b-col v-else cols="12" class="px-0">
                ...
                <b-col cols="12" class="px-0">
                    ...
                    <b-popover target="popover-button-variant" variant="info" triggers="focus">
                        ...
                        <div>
                            <img width="15" height="15" src="https://img.icons8.com/ios/50/select-none.png"
                                alt="select-none" />
                ...
                <b-col>
                    <SentimentUsers :filters="submittedFilters" @filterAccount="updateAccount" />
                </b-col>
                <b-col class="mb-3 px-0">
                    <TrendSummary :summary="summary" :filters="formattedFilters" :dates="dates"
            ...
            <b-col cols="12" class="px-0">
                ...
                <div v-if="!loadingTimeline && !loadingChart">
                    <ExportExcelButton :posts="postForExport" :filters="formFilters"
                        :disabled="loadAllPost || (Array.isArray(postForExport) && postForExport.length === 0)"
                ...
                <vue-element-loading :active="loadingTimeline" class="h-100" size="80"
                ...
                <b-col v-if="totalPage > 1 && !loadingTimeline && !loadingChart" class="p-0 mb-5">
                    <b-row v-if="page !== totalPage && (totalPage > page)"
...
import TrendSummary from '@/components/monitorlist/TrendSummary.vue'
export default {
    components: {
        Timeline,
        GroupMembers,
    ...
    computed: {
        ...
        formattedFilters() {
            const f = this.submittedFilters;

            const start = f.from ? `${f.from}T00:00:00` : null;
            const end   = f.to   ? `${f.to}T23:59:59` : null;
                ...
                const { data } = await this.axios.get(API_URL, {
                    params: {
                        ...
                        sentiment: Array.isArray(this.formFilters?.sentiment)
                            ? this.formFilters.sentiment.join(',')
                            : this.formFilters?.sentiment ?? '',
                        from: this.valueDate?.[0] +'T00:00:00' ?? null,
                        to: this.valueDate?.[1]+ 'T23:59:59' ?? null,
                        page: this.page,
                        limit: this.limit,
